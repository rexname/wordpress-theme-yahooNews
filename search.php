...
<main class="mx-auto max-w-6xl px-4 py-6">
  <header class="mb-6">
    <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 md:text-3xl">
      <?php
      if ($hasSearchQuery) {
          echo esc_html('Search results for: ' . $searchQuery);
      } else {
          echo 'Search';
      }
      ?>
  ...
  $topQueryArgs = [
      'post_type' => 'post',
      'posts_per_page' => $hasSearchQuery ? 4 : 0,
      'ignore_sticky_posts' => true,
      'orderby' => 'date',
      'order' => 'DESC',
  ];
  if ($hasSearchQuery) {
      $topQueryArgs['s'] = $searchQuery;
  }

  $topQuery = new WP_Query($topQueryArgs);

  $top = [];
  if ($topQuery->have_posts()) {
      while ($topQuery->have_posts()) {
          $topQuery->the_post();
          $id = get_the_ID();
          $cat = yahooNews_post_primary_category($id);
          $top[] = [
              'id' => $id,
              'title' => get_the_title(),
              'link' => get_permalink(),
              'time' => human_time_diff((int) get_the_time('U'), (int) current_time('timestamp')) . ' ago',
              'cat' => $cat instanceof WP_Term ? $cat->name : 'Stories',
  ...
  $storiesQueryArgs = [
      'post_type' => 'post',
      'posts_per_page' => $hasSearchQuery ? 5 : 0,
      'offset' => $topCount,
      'ignore_sticky_posts' => true,
      'orderby' => 'date',
      'order' => 'DESC',
  ...
  if ($hasSearchQuery) {
      $storiesQueryArgs['s'] = $searchQuery;
  }

  $storiesQuery = new WP_Query($storiesQueryArgs);

  $storiesInitialCount = (int) $storiesQuery->post_count;
  $storiesNextOffset = $topCount + $storiesInitialCount;
  ?>
          ...
          <a class="group relative block h-[420px] overflow-hidden rounded-2xl bg-slate-100" href="<?php echo esc_url($top[0]['link']); ?>">
            ...
            <div class="absolute inset-x-0 bottom-0 p-6">
              <div class="flex items-center gap-2 text-xs font-semibold text-white/90">
                <span class="rounded-full bg-white/15 px-2 py-1"><?php echo esc_html($top[0]['cat']); ?></span>
            ...
            <a class="group relative block h-[210px] overflow-hidden rounded-2xl bg-slate-100" href="<?php echo esc_url($top[1]['link']); ?>">
              ...
              <div class="absolute inset-x-0 bottom-0 p-4">
                <div class="flex items-center gap-2 text-[11px] font-semibold text-white/90">
                  <span class="rounded-full bg-white/15 px-2 py-1"><?php echo esc_html($top[1]['cat']); ?></span>
                  <span class="text-white/70">•</span>
...
<div class="mt-10 grid gap-6 lg:grid-cols-12">
  <section class="lg:col-span-9">
    <div class="mb-4 flex items-center gap-2">
      ...
      <h2 class="text-sm font-extrabold text-slate-900">Search results</h2>
    </div>

    <div id="stories-list" class="space-y-5" data-search-query="<?php echo esc_attr($searchQuery); ?>" data-next-offset="<?php echo esc_attr((string) $storiesNextOffset); ?>">
      <?php if ($storiesQuery->have_posts()) : ?>
        <?php while ($storiesQuery->have_posts()) : ?>
          <?php $storiesQuery->the_post(); ?>
          <?php $cat = yahooNews_post_primary_category(); ?>
          <?php $minutes = yahooNews_post_read_time_minutes(); ?>
          <?php $ts = (int) get_the_time('U'); ?>
      ...
      <?php else : ?>
        <div class="rounded-2xl border border-slate-200 bg-white p-5">
          <?php if ($hasSearchQuery) : ?>
            <div class="text-sm font-extrabold text-slate-900">No results found.</div>
            <div class="mt-1 text-sm text-slate-600"><?php echo esc_html('Query: ' . $searchQuery); ?></div>
          <?php else : ?>
            <div class="text-sm font-extrabold text-slate-900">No search query provided.</div>
          <?php endif; ?>
        </div>
      <?php endif; ?>
    </div>
